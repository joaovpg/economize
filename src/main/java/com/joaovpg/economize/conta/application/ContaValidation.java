...
final class ContaValidation {
    ...
    static BigDecimal saldoInicial(BigDecimal saldoInicial) {
        if (saldoInicial == null || saldoInicial.scale() > 4 || saldoInicial.setScale(4).precision() > 19) {
    ...
    static LocalDate dataSaldoInicial(LocalDate dataSaldoInicial, LocalDate dataAtual) {
        if (dataSaldoInicial == null || dataSaldoInicial.isAfter(dataAtual)) {
            throw new RegraNegocioException("DATA_SALDO_INICIAL_INVALIDA", "Data do saldo inicial invalida");
        }
    ...
    static void nomeDisponivel(ContaFinanceiraRepository repository, UUID usuarioId, String nome) {
        if (repository.existeComNome(usuarioId, nome)) {
            throw new RegraNegocioException("NOME_CONTA_DUPLICADO",
                    "Ja existe uma conta com esse nome");
        }
    }

    static void nomeDisponivel(ContaFinanceiraRepository repository, UUID usuarioId,
                               String nome, UUID ignorarId) {
    ...
    static void dadosIniciaisEditaveis(ContaFinanceira conta, String moeda,
                                       BigDecimal saldoInicial, LocalDate dataSaldoInicial) {
        if (conta.isDadosIniciaisBloqueados()
            ...
            throw new RegraNegocioException("DADOS_INICIAIS_CONTA_IMUTAVEIS",
    ...
    static ContaResultado resultado(ContaFinanceira conta) {
        return new ContaResultado(conta.getId(), conta.getNome(), conta.getMoeda(), conta.getSaldoInicial(),
                conta.getDataSaldoInicial(), conta.isAtivo() ? SituacaoConta.ATIVA : SituacaoConta.INATIVA);
    }
}
