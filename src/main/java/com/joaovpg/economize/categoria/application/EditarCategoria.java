...
public class EditarCategoria {
    ...
    public CategoriaResultado executar(Comando comando) {
        var categoria = categoriaRepository.buscarDoUsuario(comando.categoriaId(), comando.usuarioId())
                .orElseThrow(CadastrarCategoria::naoEncontrada);
        if (comando.situacao() == null) {
            throw new RegraNegocioException("SITUACAO_CATEGORIA_INVALIDA", "Situacao da categoria invalida");
        }
        ...
        CategoriaValidation.nomeDisponivel(
                categoriaRepository, comando.usuarioId(), comando.categoriaPaiId(), nome, categoria.getId());
    ...
    private Categoria buscarEValidarPai(Comando comando, Categoria categoria) {
        ...
        var pai = categoriaRepository.buscarDoUsuario(comando.categoriaPaiId(), comando.usuarioId())
                .orElseThrow(CadastrarCategoria::naoEncontrada);
        boolean mesmoPai = categoria.getCategoriaPai() != null && categoria.getCategoriaPai().getId().equals(pai.getId());
    ...
    private void validarSituacao(Categoria categoria, Categoria novoPai, SituacaoCategoria situacao) {
        ...
        if (situacao == SituacaoCategoria.INATIVA && categoriaRepository.existeDescendenteAtiva(categoria.getId())) {
            throw new RegraNegocioException("CATEGORIA_POSSUI_DESCENDENTE_ATIVA",
                    "A categoria possui descendente ativa");
        }
        if (situacao == SituacaoCategoria.ATIVA) {
            for (var ancestral = novoPai; ancestral != null; ancestral = ancestral.getCategoriaPai()) {
                if (CategoriaValidation.situacao(ancestral) == SituacaoCategoria.INATIVA) {
                    throw new RegraNegocioException("CATEGORIA_POSSUI_ANCESTRAL_INATIVA",
                            "Todos os ancestrais devem estar ativos");
                }
            }
        }
    }
    ...
    private RegraNegocioException ciclo() {
        return new RegraNegocioException("HIERARQUIA_CATEGORIA_CICLICA", "A hierarquia da categoria formaria um ciclo");
    }

    public record Comando(UUID usuarioId, UUID categoriaId, String nome, String cor,
                          UUID categoriaPaiId, SituacaoCategoria situacao) {}
}
