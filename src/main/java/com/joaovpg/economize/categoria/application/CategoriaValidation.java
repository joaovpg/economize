...
final class CategoriaValidation {
    ...
    static void nomeDisponivel(CategoriaRepository repository, UUID usuarioId, UUID paiId,
    ...
    static CategoriaResultado resultado(Categoria categoria) {
        return new CategoriaResultado(categoria.getId(), categoria.getNome(), categoria.getCor(),
    ...
    private static RegraNegocioException nomeDuplicado() {
        return new RegraNegocioException("NOME_CATEGORIA_DUPLICADO",
                "Ja existe uma categoria com esse nome no mesmo nivel");
    }
}
