...
public class AutenticarUsuario {
    private static final String HASH_COMPARACAO = Argon2Factory.create()
            .hash(2, 19_456, 1, "credencial-inexistente".toCharArray());
    ...
    public Resultado executar(Comando comando) {
        var usuario = usuarioRepository.buscarPorEmail(comando.email())
        ...
        try {
            var senhaValida = argon2.verify(usuario.map(Usuario::getSenhaHash).orElse(HASH_COMPARACAO), senha);
        ...
        var token = Jwt.subject(usuario.orElseThrow().getId().toString()).groups(Set.of("usuario")).sign();
